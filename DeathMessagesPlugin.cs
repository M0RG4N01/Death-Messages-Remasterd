...
using RocketMod.Plugins.DeathMessages.Configuration;
using SDG.Unturned;
using Steamworks;
using UnityEngine;
using Logger = Rocket.Core.Logging.Logger;

namespace RocketMod.Plugins.DeathMessages;

public sealed class DeathMessagesPlugin : RocketPlugin<DeathMessagesConfiguration>
{
    ...
    protected override void Load()
    {
        ...
        Logger.Log(Configuration.Instance.SuicideMessages
            ? "Suicide messages are enabled!"
            : "Suicide messages are disabled!", ConsoleColor.Yellow);

        Logger.Log(Configuration.Instance.HealthWarningMessages
            ? "Health warning messages are enabled!"
            : "Health warning messages are disabled!", ConsoleColor.Yellow);

        Logger.Log(Configuration.Instance.ExperienceRewardsEnabled
            ? "Experience is enabled!"
            : "Experience is disabled!", ConsoleColor.Yellow);

        Logger.Log(Configuration.Instance.ZombieMessages
            ? "Zombie Death Messages are enabled!"
            : "Zombie Death Messages are disabled!", ConsoleColor.Yellow);
...

    {
        ...
        var msgColour = UnturnedChat.GetColorFromName(Configuration.Instance.MessageColour, Color.red);
        ...
        if (!Configuration.Instance.ExperienceRewardsEnabled)
            experienceReward = 0;
        ...
        {
            var includeLimbWord = cause is EDeathCause.PUNCH or EDeathCause.MELEE or EDeathCause.GUN &&
                                  limb == ELimb.SKULL;
            var vehicleName = "N/A";
            var itemName = "N/A";
            var killDistance =
            ...
            if (murderer?.CurrentVehicle != null)
                vehicleName = murderer.CurrentVehicle.asset?.vehicleName ?? "N/A";
        ...
        if (moneyReward != 0 && Uconomy.Instance != null)
            try
            {
